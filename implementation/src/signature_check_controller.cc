...
void send_breakpoint_signal(int idx) {
    auto fd = xdr::tcp_connect(hostname_from_idx(idx).c_str(), SIGCHECK_PORT);
    auto client = xdr::srpc_client<SignatureCheckingV1>(fd.get());
    client.signal_start();
}
...
bool node_is_ready(int idx) {
    try {
        ...
        return false;
        //return client.is_ready_to_start() == 1;
    } catch(...) {
...
void wait_for_all_nodes_ready(int num_replicas) {
    ...
    //std::printf("node 1 online\n");
    //wait_for_node_ready(2);
    //std::printf("node 2 online\n");
    //wait_for_node_ready(3);
//  std::printf("node 3 online\n");
//  wait_for_node_ready(4);
//  std::printf("node 4 online\n");
}
...
int main(int argc, char const *argv[]) {
    ...
    if (argc < 3 || argc > 4) {
        std::printf("usage: ./signature_check_controller output_folder num_replicas <skip_intro> \n");
        return 0;
    }

    std::string base_name = std::string("signature_check_results/");
