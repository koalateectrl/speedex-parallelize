...
public:
	SyntheticDataGenSigner() 
		: key_gen() {
			...
			//auto [sks, pks] = key_gen.gen_key_pair_list(num_accounts);
			//keys = sks;
		}
	...
	void add_account(AccountID new_account) {
		...
		//for (size_t i = keys.size(); i <= max_account; i++) {
		//	auto [sk, _] = key_gen.deterministic_key_gen(i);
		//	keys.push_back(sk);
		//}
	}
...
struct GeneratorState {
	...
	//std::unordered_map<uint64_t, AccountID> existing_accounts_map;
...
//		AccountID out = num_active_accounts;
		num_active_accounts ++;
