...
class CycleDistribution {
	...
	void create_acc_probabilities(const double* individual_scores, const int individual_scores_size);

public:

	int get_size(double random) const; //takes rand in [0,1]
	bool parse(struct fy_document* fyd, int num_assets);
};
...
struct PriceOptions {
	...
	bool parse(struct fy_document* fyd);
};
