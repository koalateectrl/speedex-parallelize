...
uint64_t increment_step(const uint64_t step, const uint16_t step_up, const uint16_t step_adjust_radix) {
	...
	//std::printf("increment step: %lu %u %u %lu\n", step, step_up, step_adjust_radix, increment);
...
TatonnementOracle::get_objective(const uint128_t* supplies, const uint128_t* demands, const Price* prices, const ObjectiveFunctionInputs& inputs) {
	...
	*/
	long double acc = 0;
	for (size_t i = 0; i < num_assets; i++) {
		long double diff = PriceUtils::amount_to_double(supplies[i], PriceUtils::PRICE_RADIX) - PriceUtils::amount_to_double(demands[i], PriceUtils::PRICE_RADIX);//(double) supplies[i] - (double) demands[i];
...
Price get_trial_price(const uint128_t& demand, const uint128_t& supply, const Price& old_price, const uint64_t& step, const uint16_t volume_relativizer, const TatonnementControlParameters& control_params) {
	...
	if (demand > supply) {
		...
		Price delta = PriceUtils::safe_multiply_and_drop_lowbits(p_times_step, p_times_diff, control_params.step_radix + PriceUtils::PRICE_RADIX);

		return PriceUtils::impose_bounds(old_price + delta);
	} else {
...
void TatonnementOracle::get_supply_demand(
	...
	ObjectiveFunctionInputs& function_inputs) {
...
void TatonnementOracle::run_tatonnement_thread(TatonnementControlParameters* control_params_ptr_) {
	...
	while(true) {
		...
		num_active_threads ++;
//		num_active_threads.fetch_add(1);
		lock.unlock();
		...
		//std::printf("num active threads: %u\n", num_active_threads);
...
void TatonnementOracle::start_tatonnement_threads() {
	size_t num_work_units = WorkUnitManagerUtils::get_num_work_units_by_asset_count(num_assets);

	//TatonnementControlParameters params(num_assets, num_work_units);
...
TatonnementOracle::normalize_prices(
	...
	int space_to_top = __builtin_clzll(p_max) - (64 - PriceUtils::PRICE_BIT_LEN);
	...
	while (true) {
		...
		if (clearing) {
			...
			if (!not_first_clear) {
				...
				TAT_INFO("ROUND %5d step_size %lf (%lu) objective %lf", round_number, PriceUtils::amount_to_double(step, step_radix), step, prev_objective.l2norm_sq);
				TAT_INFO("Asset\tdemand\tsupply\tprice\ttax revenue\toversupply");
				for (size_t i = 0; i < num_assets; i++) {
					double demand = PriceUtils::amount_to_double(demands_search[i], PriceUtils::PRICE_RADIX);
					demand = demand - PriceUtils::amount_to_double(demands_search[i], PriceUtils::PRICE_RADIX + active_approx_params.tax_rate);
					double delta = demand - PriceUtils::amount_to_double(supplies_search[i], PriceUtils::PRICE_RADIX);
					TAT_INFO("%d\t%010.3f\t%010.3f\t%010.3f\t(%8lu)\t\t%010.3f\t%010.3f", 
						...
						PriceUtils::to_double(prices_workspace[i]),
						prices_workspace[i],
						PriceUtils::amount_to_double(demands_search[i], PriceUtils::PRICE_RADIX + active_approx_params.tax_rate),
						delta);
				}
...
//		auto new_objective = get_objective(supplies_workspace, demands_workspace, prices_workspace, function_inputs);
		...
		if (round_number % 100000 == -1) {
			TAT_INFO("ROUND %5d step_size %lf (%lu) objective %lf step_radix %lu", round_number, PriceUtils::amount_to_double(step, step_radix), step, prev_objective.l2norm_sq, step_radix);
			//TAT_INFO("Asset\tdemand\tsupply\tprice\ttax revenue\toversupply");
			for (size_t i = 0; i < num_assets; i++) {
				double demand = PriceUtils::amount_to_double(demands_search[i], PriceUtils::PRICE_RADIX);
				demand = demand - PriceUtils::amount_to_double(demands_search[i], PriceUtils::PRICE_RADIX + active_approx_params.tax_rate);
