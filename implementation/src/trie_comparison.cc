...
float test_merkle_trie_emptyvalue(const std::vector<AccountID>& accounts) {
	...
	using TrieT = MerkleTrie<8, EmptyValue, LogMetadataT>;
	auto timestamp = init_time_measurement();
	using prefix_t = TrieT::prefix_t;
	prefix_t prefix_buffer;
	TrieT trie;
	for (auto& account : accounts) {
		PriceUtils::write_unsigned_big_endian(prefix_buffer, account);
...
float test_merkle_trie(const std::vector<AccountID>& accounts) {
	using LogMetadataT = CombinedMetadata<SizeMixin>;
	using TrieT = MerkleTrie<8, ValueT, LogMetadataT>;
	auto timestamp = init_time_measurement();
	using prefix_t = TrieT::prefix_t;
	prefix_t prefix_buffer;
	ValueT value_buffer;
